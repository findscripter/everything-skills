...
- gemini-extension.json retry：TheQtCompanyRnD/agent-skills 已入表；多数命中为应用扩展而非技能库
...
- skills-101/superpowers（obra/superpowers 镜像嫌疑，本轮 search 0 hits，仍未核验 README）
- konnga/skill-buddy（V2EX 技能管理器，非技能包）
- InternScience/Awesome-Scientific-Skills（根 README 仍不存在）
- NVIDIA/TileGym（CUDA Tile 内核教程库，非 Agent Skills）
- getsentry/warden-skills（Warden 产品技能，非通用编码代理包）
- withastro/skills、expo/tuft-skills（0–2★ 过小）
- gsd-build/get-shit-done（已归档，README 仅跳转，仍未核验后继仓）
- leenkcool/Blue-Whale-Harness（DeepSeek 插件总目录，本轮未入表）
- loulanyue/skills、tamdogood/builder-essential-skills（个人 dump，本轮未入表）
- Owl-Listener/ux-pgm-skills、design-leadership-skills（2–14★，套件附属过小）
- merill/msgraph（单技能 Graph）
- skrun-dev/skrun（技能运行时，非技能库）
- K-Dense-AI/mimeo（生成器，非技能包）；drug-discovery-agent-skills（15★）
- BehiSecc/VibeSec-Skill（单技能）；jthack/ffuf 等攻击向跳过
- rockscy/solo-skills（8★）
- antonbabenko/agent-plugins（40★ 市场，terraform-skill 已入表）
- stripe/ai（Stripe AI 产品套件，非 SKILL.md 技能库）
- marswaveai/skills（78★ ListenHub，本轮未入表）
