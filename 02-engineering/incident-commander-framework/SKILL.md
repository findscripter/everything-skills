...
### Runbook 骨架（新服务 on-call 必备）

为每个关键服务维护检测 / 止损 / 恢复三类条目，最少包含：

```
# {服务} 事故响应 Runbook
## 速查：值班联系 / 仪表盘 / 状态页
## 检测：告警名、手动征兆
## 初始响应（0–15 分钟）：定级、开单、建战时频道、查最近发布
## 止损策略：回滚 / 限流 / failover（各写前置条件与风险）
## 恢复验证：核心 SLI、端到端检查、宣告解决标准
## 常见坑：级联依赖、缓存脏数据、半发布状态
```
...
采编自 alirezarezvani/claude-skills（MIT 许可）。2026-09-16 上游刷新（吸收上游新增程序要点，保持 SCHEMA 中文适配密度）。
