...
A curated, adapted collection. Per-skill licenses vary — see each `SKILL.md` `source_license` and `INDEX/sources.md`; overall terms in the root [`LICENSE`](../LICENSE) and [`NOTICE`](../NOTICE).

---

中文版（主版本）见仓库根目录。This English tree mirrors the Chinese root one-to-one by skill `name`.
