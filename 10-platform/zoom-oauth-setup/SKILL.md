...
### 企业预审批与 Active Apps Notifier（AAN）

- **Pre-Approval**：部分 Zoom 账户要求 Marketplace 管理员预审批后用户才能授权应用；账户级（admin/master）scope 还需相应角色权限。集成设计文档应写明租户是否启用预审批。
- **AAN（Active Apps Notifier）**：会中展示正在实时访问内容的应用（图标+提示：应用信息、访问内容类型、批准账户）。客户端 ≥5.6.7、Meeting SDK ≥5.9.0。面向会中内容访问的应用需考虑此可见性与合规沟通。
...
—— 本条采编自 anthropics/knowledge-work-plugins（Apache-2.0 许可）。2026-09-16 上游刷新（吸收上游新增程序要点，保持 SCHEMA 中文适配密度）。
