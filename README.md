...
>
> 本库 1108 条中文技能；另索引 **1007** 个外部 GitHub 技能库（只读 README）。
>
...
目前索引 **1007** 个 GitHub 技能库/市场/精选列表。只根据 README 摘要，不收录对方源码、不复制 SKILL.md。

完整分表（官方 / 精选 / 垂直 / 基建 / 其他 / 无名）由 `data/skill-repos.jsonl`（及 part 分片）生成，见 **[INDEX/skill-repos.md](INDEX/skill-repos.md)**。本页不再维护千级链接清单，以免与 jsonl 脱节。
