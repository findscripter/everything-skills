...
- **IV percentile / rank**：把当前 IV 放进近 1 年 HV 或 IV 序列看分位——常见启发式：>75 分位偏贵（倾向卖方收权利金），<25 分位偏便宜（倾向买方）；须同时看事件与流动性。
- **财报策略对照**：买跨式/勒式仅当预期实现波动 **大于** 隐含波幅 √(DTE/365)·IV·S；否则卖方结构（如铁鹰）更能吃 IV crush，但须硬止损防跳空出区间。
...
采编自 tradermonty/claude-trading-skills（MIT 许可）：options-strategy-advisor。本条为适配重写而非逐字翻译，保留其 Black-Scholes 定价/希腊值公式与实现、到期损益模拟逻辑、盈亏图思路、IV crush 警示、隐含波幅与仓位规模算法、各策略离场规则等关键约束。2026-09-16 上游刷新（吸收上游新增程序要点，保持 SCHEMA 中文适配密度）。
