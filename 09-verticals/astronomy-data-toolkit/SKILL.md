---
name: astronomy-data-toolkit
title: 天文数据分析（Astropy）
description: 当用 Python 处理天文/天体物理数据（坐标变换、单位换算、FITS 读写、表格、时间系统、WCS、宇宙学）时使用；用 Astropy 编写或调试分析代码并产出量纲一致、可复现的结果；不适用于通用数据清洗或非天文数值计算（改用 pandas/numpy）。触发词：astropy、天文数据、astronomy、SkyCoord、坐标变换、FITS、WCS、宇宙学、cosmology、天文单位、Quantity
domain: 领域/science
triggers: [astropy, 天文数据, astronomy, SkyCoord, 坐标变换, FITS, WCS, 宇宙学, cosmology, 天文单位, Quantity]
tags: [astropy, astronomy, fits, coordinates, cosmology, wcs, units, python, science]
level: 进阶
status: stable
...
requires: []
related: [materials-science-toolkit, cheminformatics-toolkit, sympy-symbolic-math]
...
1. 安装并锁版本（可复现）：`uv pip install "astropy==7.2.0"`；需绘图等可选依赖时用 `"astropy[recommended]==7.2.0"`，广覆盖才用 `[all]`。要求 Python 3.11+，依赖 NumPy、PyERFA、PyYAML；用隔离虚拟环境，勿以管理员权限安装。
2. 按需导入子模块（见「指令」）。
3. **凡数值带单位的量一律附单位**（用 `* u.<unit>`），换算用 `.to()`，保证量纲一致。
4. FITS 文件用 `with fits.open(...)` 上下文管理器确保关闭；大文件可用内存映射。
5. 批量坐标/时间用数组而非循环，性能更好。
6. 做坐标变换前先确认源 frame；AltAz 需提供 `obstime` 与 `EarthLocation`。
7. 宇宙学选定合适模型（如 `Planck18`）；有单位的表用 `QTable`；缺失值用掩码列。
...
- **网络访问要显式**：`SkyCoord.from_name()`、`EarthLocation.of_site(refresh_cache=True)`、`EarthLocation.of_address()`、`download_file()`、远程 FITS 读取及部分 IERS 时间/坐标转换会联网或更新本地缓存。勿把敏感目标名、地址、URL、专有文件路径发往第三方服务。
- **可复现**：共享环境锁版本（如 `astropy==7.2.0`），更新前先看 release notes。
...
- 时标要显式（UTC/TT/TDB），高精度计时尤其重要；用 WCS 转换前先校验其有效性；昂贵计算（如宇宙学距离）可缓存。
- 当前研究的稳定版：Astropy 7.2.0（2025-11-25 发布）。

## 互见

- 官方文档：https://docs.astropy.org/en/stable/ ；教程：https://learn.astropy.org/
- 源条目附有分模块参考：`references/units.md`、`coordinates.md`、`cosmology.md`、`fits.md`、`tables.md`、`time.md`、`wcs_and_other_modules.md`（涵盖 NDData/CCDData、建模、可视化、常数、卷积、统计）。
