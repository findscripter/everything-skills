...
1. 预算方差容忍：方差 >5% 即触发管理层审查 → 偏固定；能扛 15–20% → 指数或区块+指数可行。
2. 价格周期位置：远期处 5 年区间底部 1/3 → 多锁固定；顶部 1/3 → 多留指数；不确定则分层。
3. 期限：12 个月固定 vs 指数溢价差小；36+ 个月固定溢价复利、易锁贵 → 偏混合/分层。
4. 负荷率：>0.75 适合全时段平价区块；<0.50 适合整形或 TOU 指数匹配峰谷。
...
---
采编自 affaan-m/ECC（MIT）。2026-09-16 上游刷新（吸收上游新增程序要点，保持 SCHEMA 中文适配密度）。
