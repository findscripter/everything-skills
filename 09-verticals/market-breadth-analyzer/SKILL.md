...
title: Market Breadth Analyzer Skill
...
domain: 领域/fintech
triggers: [breadth, advance-decline]
...
- User wants to assess the market participation rate
- User asks about advance-decline indicators or breadth thrust
- User wants to know whether the market is narrowing (fewer stocks participating)
- User asks about equity exposure levels based on breadth conditions

## Prerequisites

- **Python 3.8+** with the `requests` library (for fetching CSV data)
...
The script will:
1. Fetch the detail CSV (~2,500 rows, 2016-present) and the summary CSV (8 metrics)
...
4. Generate a composite score with zone classification
5. Track score history and compute the trend (improving/deteriorating/stable)
...
- **First use:** Load the methodology reference for framework understanding
- **Regular execution:** References not needed - the script handles scoring
