...
title: FoundationModels: On-Device LLM (iOS 26)
description: デバイス上基盤モデルの実装パターン、量子化、最適化、およびプライバシーを考慮した推論。
domain: 智能/model-ops
triggers: [FoundationModels, on-device, Apple Intelligence, iOS 26, SystemLanguageModel, LanguageModelSession, Generable, Guide, PartiallyGenerated, Swift, SwiftUI, 4096 token]
tags: [model-ops, apple, foundationmodels, swift]
level: intermediate
status: stable
