...
5. 设计参数并配 UI 控件：从「这个系统有哪些可调的性质」出发（数量/尺度/概率/比例/角度/频率），而不是从「图案类型」出发。
...
- 90% 算法生成 + 10% 关键参数；禁止从「图案菜单」点选后贴哲学标签。
- 若仓库含 `templates/viewer.html`：先 Read 再改，保留 Seed/Actions 固定区，只换算法与 Parameters。
- 种子导航（当前/上一个/下一个/随机/跳转）与 Regenerate/Reset/Download PNG 必须真实可用。
- 动画目标约 60fps；粒子邻域查询避免无界 O(n²)。
