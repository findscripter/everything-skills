...
模板里的彩色说明框是「占位指令」，不是成品格式。刷新/填数时必须遵守：

1. **禁止把数据填进占位彩框**：黄/橙等亮色说明框应整块删除，再按模板正文样式（通常深色字+浅色底）新建内容。识别测试：若成品仍是大块彩底填字，即误把占位格式当输出格式。
2. **两类占位要分清**：
   - 指令框（亮色 +「在此插入…」）→ **删除整形状** 后重建
   - 版式占位（母版中性色 +「单击添加文本」）→ **保留形状，只替换文字**
3. **禁止文本伪表**：不得用 `|`/Tab/空格对齐冒充表格；必须用真正的 table 对象。
4. **禁止继承占位对比度**：占位常用浅色字配彩底以示「请替换」；成品正文通常为深字浅底，对比度必须可读。
5. **先删占位 → 填内容 → 再套模板格式**；图表/箭头用 PPT 对象而非位图糊墙。
...
---
采编自 anthropics/financial-services（Apache-2.0）。2026-09-16 上游刷新（吸收上游新增程序要点，保持 SCHEMA 中文适配密度）。
